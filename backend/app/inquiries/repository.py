...
class QuotationRepository(BaseRepository[Quotation]):
    ...
    async def get_all_quotation_documents(self) -> list[dict]:
        """
        Fetch every quotation that has an attached file (PDF/image), joined
        with its product and supplier metadata, for the Product & Supplier
        Gallery's quotation-documents view.

        Only quotations with ``attachment_url`` set are returned -- a quote
        entered manually with no file has nothing to show in a document
        gallery.
        """
        stmt = (
            select(
