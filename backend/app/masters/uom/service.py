...
class UomService:
    ...
    async def create(self, **field_values: Any) -> UnitOfMeasurement:
        ...
        if name and await self.repository.name_exists(name):
            raise ConflictException(f"UOM name {name!r} is already in use.")
        if code and await self.repository.code_exists(code):
            raise ConflictException(f"UOM code {code!r} is already in use.")
    ...
    async def update(self, uom_id: uuid.UUID, **field_values: Any) -> UnitOfMeasurement:
        ...
        name = field_values.get("name")
        code = field_values.get("code")
        if name and await self.repository.name_exists(name, exclude_id=uom_id):
            raise ConflictException(f"UOM name {name!r} is already in use.")
        if code and await self.repository.code_exists(code, exclude_id=uom_id):
            raise ConflictException(f"UOM code {code!r} is already in use.")
    ...
    async def import_file(self, filename: str, raw_bytes: bytes) -> ImportSummary:
        ...
        async def _create(field_values: dict[str, Any]) -> UnitOfMeasurement:
            ...
            if await self.repository.name_exists(name):
                raise ValueError(f"UOM name {name!r} already exists.")
            if await self.repository.code_exists(code):
                raise ValueError(f"UOM code {code!r} already exists.")
